...
def wait_for_pid(pid: int, timeout_ms: int = 45000) -> None:
    ...
    if not handle:
        # O processo já pode ter encerrado entre o Popen e esta chamada.
        time.sleep(0.8)
        return
    try:
...
def replace_file_with_retry(src: Path, dst: Path, timeout: float = 25.0) -> None:
    """Substitui um arquivo do programa aguardando locks residuais do Windows.

    PyInstaller onefile pode deixar o executável mapeado por alguns instantes
    depois da janela fechar. O atualizador anterior fazia apenas uma tentativa.
    """
    ...
    if actual != expected:
        raise RuntimeError(
            f"A conferência de integridade de {dst.name} falhou após a substituição."
        )
...
def install_package(target: Path, package: Path, current_version: str) -> tuple[Path, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="PlanilhadorUpdate_") as temp:
        ...
        # Confirma mais uma vez antes de reiniciar: não basta o replace não ter
        # gerado exceção; o EXE final precisa ser exatamente o do pacote novo.
...
def main() -> int:
    ...
    try:
        ...
        # Volta à versão preservada, mas o novo Planilhador (quando já instalado)
        # sabe não oferecer automaticamente a mesma versão que falhou.
        try:
            restart(target)
        except Exception:
            pass
